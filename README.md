...
Если `planner.py` не возвращает корректный JSON или не может построить расписание, адаптер возвращает соответствующую ошибку.
...
3. При невозможности получить корректный JSON или если планировщик возвращает `None` (не удалось построить расписание), адаптер возвращает соответствующую ошибку HTTP.
...
* При отсутствии корректного JSON из `planner.py` или невозможности построить расписание возвращается соответствующая ошибка HTTP.
